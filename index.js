...
function updateHomepage() {
    ...
    document.getElementById("nightHours").innerText =
        `${divideWithRemainder(getData()[2], 60)[0]}hr ${divideWithRemainder(getData()[2], 60)[1]}min night`;
...
document.addEventListener("DOMContentLoaded", () => {
    // Show loading screen if needed. Use dark mode session storage to tell
    if (sessionStorage.getItem("darkMode") === null) {
        // Show loading screen
        document.getElementById("loading").hidden = false;

        // Hide loading screen after a second
        setTimeout(() => {
            document.getElementById("loading").style.opacity = 0;

            // Update homepage as loading screen is fading
            updateHomepage();

            // Hide loading screen after animation is finished
            setTimeout(() => {
                document.getElementById("loading").hidden = true;
            }, 300);
        }, 1000);
    } else {
        // Just update homepage without loading screen
        updateHomepage();
    }
